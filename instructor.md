...
`create_table.py`调用`db_connection.py`中的函数实现建表和约束、触发器设置。

初始化管理员：

`set_admin.py`调用`db_connection.py`中的函数初始化两位管理员：`("Admin1", "111111", "Test Admin A")`和`("Admin2", "222222", "Test Admin B")`。
